...
// Total de economia potencial na conta
...
<template>
  <div class="quotations">
    ...
    <div class="quotations__header">
      ...
      <button
        class="btn-primary"
        @click="router.push({ name: 'quotation-create' })"
      >
        + Nova cotação
      </button>
    </div>
    ...
    <div v-else class="groups-list">
      ...
      >
        ...
        <div
          ...
          @click="toggleExpand(group.productName)"
        >
          <div class="group-card__info">
            <span class="group-card__name">{{ group.productName }}</span>
            <span class="group-card__count">{{ group.quotationCount }} cotação{{ group.quotationCount !== 1 ? 'ões' : '' }}</span>
          </div>
            ...
            >
              <div class="quotation-row__info">
                <span class="quotation-row__supplier">
                  <span v-if="index === 0 && group.quotationCount > 1" class="best-badge">✓ Mais barato</span>
                  {{ q.supplier }}
                </span>
                <span class="quotation-row__meta">
                  {{ formatDate(q.quotationDate) }}
                  <span v-if="q.farmName"> · {{ q.farmName }}</span>
                  <span v-if="q.notes"> · {{ q.notes }}</span>
                </span>
              </div>
              ...
              <div class="quotation-row__prices">
                <div class="quotation-row__price-block">
                  ...
                  <span class="quotation-row__unit-label">por {{ q.unit || 'un' }}</span>
                </div>
                <div class="quotation-row__price-block">
                  <span class="quotation-row__qty">{{ Number(q.quantity).toLocaleString('pt-BR') }} {{ q.unit || 'un' }}</span>
              ...
              <div class="quotation-row__actions">
                <button
                  class="icon-btn"
                  title="Editar"
                  @click.stop="router.push({ name: 'quotation-edit', params: { quotationId: q.id } })"
                >
...
.savings-banner__icon { font-size: 1.5rem; flex-shrink: 0; }
...
.quotation-row--cheapest .quotation-row__total {
  color: #14532d;
}

.quotation-row--cheapest .best-badge {
  background: #bbf7d0;
  color: #14532d;
}

.quotation-row--cheapest .icon-btn {
  background: #dcfce7;
  border-color: #86efac;
  color: #15803d;
}

.quotation-row--cheapest .icon-btn:hover {
  background: #bbf7d0;
  border-color: #4ade80;
  color: #14532d;
}
